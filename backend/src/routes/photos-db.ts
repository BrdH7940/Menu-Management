...
photosDbRouter.get('/:id/photos', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Không thể lấy danh sách ảnh' });
  }
});

// POST /api/admin/menu/items/:id/photos - Upload ảnh
photosDbRouter.post('/:id/photos', upload.array('photos', 10), async (req, res) => {
  try {
    const menuItemId = req.params.id;
...
photosDbRouter.patch('/:id/photos/:photoId/primary', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Không thể đặt ảnh làm ảnh chính' });
  }
});
